...
public:  
    ...
    Fast_border_set(std::vector<long long> & arr) : Fast_border_set(*std::min_element(arr.begin(), arr.end()), *std::max_element(arr.begin(), arr.end())) {
        for(auto i: arr){
            data[i - this->min_el]++;
        }    
    }
    Fast_border_set(std::list<long long> & list) : Fast_border_set(*std::min_element(list.begin(), list.end()), *std::max_element(list.begin(), list.end())) {
        for(auto i: list){
            data[i - this->min_el]++;
        }    
    }

    Fast_border_set(const Fast_border_set &B){
        min_el = B.min_el;
        max_el = B.max_el;
        for(auto i: B.data){
    ...
    ~Fast_border_set(){
        max_el = min_el - 1; //non-valid value, to detect errors
        data.clear();
        data.shrink_to_fit();
    }
